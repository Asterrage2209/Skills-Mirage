...
const LatestJobs = () => {
    ...
    if (loading) {
        return <div className="card mt-8 p-4 text-center text-textSecondary">Loading latest jobs...</div>;
    }
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-gray-800 text-textSecondary text-sm font-medium">
                            <th className="pb-3 pr-4 whitespace-nowrap">Job Title</th>
                            <th className="pb-3 pr-4 whitespace-nowrap">Company</th>
                            <th className="pb-3 pr-4 whitespace-nowrap">City</th>
                            <th className="pb-3 whitespace-nowrap">Post Date</th>
                        </tr>
                    </thead>
                    <tbody className="text-sm">
                        {jobs.map((job, idx) => (
                            <tr key={idx} className="border-b border-gray-800/50 hover:bg-gray-800/20 transition-colors">
                                <td className="py-4 pr-4 text-white font-medium max-w-[250px] truncate" title={job.jobtitle}>
                                    {job.jobtitle}
                                </td>
                                <td className="py-4 pr-4 text-gray-300 max-w-[200px] truncate" title={job.company}>
                                    {job.company}
                                </td>
